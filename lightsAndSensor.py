...
if __name__ == '__main__':
    ...
    colors = [fading, flashing, red, green, blue]
    for code in generateCodes(nHigh=8, nLong=17):
        # Change color
        ...
        # Send arduino 
        onLight = getLightSensor(sensor)
        print('Light value while on: ', onLight);

        print('Turning off lights...')
        sendRadioCode(off, radio)
        time.sleep(.5)
        resetLightSensor(sensor)
        offLight = getLightSensor(sensor)
        print('Light value while off: ', offLight);
        resetLightSensor(sensor)

        if onLight > threshold:
